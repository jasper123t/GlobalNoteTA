...
function parseGlobalTables(phpContent) {
  const globalTables = {};
  const constNames = ['ZH_TO_HANT', 'ZH_TO_HANS', 'ZH_TO_TW', 'ZH_TO_HK', 'ZH_TO_CN'];
  const keyMap = {
    'ZH_TO_HANT': 'hant',
    'ZH_TO_HANS': 'hans',
    'ZH_TO_TW': 'tw',
  ...
  for (const constName of constNames) {
    const regex = new RegExp(`public const ${constName} = \\[([\\s\\S]*?)\\];`, 'm');
...
function parseTable(content, variant) {
  ...
  console.log(`Total blocks for ${variant}: ${blockCount}, total mappings: ${Object.keys(table).length}`);
  return table;
}
...
chrome.runtime.onStartup.addListener(() => {
  chrome.storage.local.get(['lastUpdated'], (result) => {
    const now = Date.now();
    if (!result.lastUpdated || now - result.lastUpdated > 24 * 60 * 60 * 1000) { // Refresh daily
      loadTables();
    }
  });
});
