...
function loadPref() {
  chrome.storage.local.get(
    ["currentVariant", "conversionEnabled", "showOriginal", "highlightEnabled"],
    (result) => {
    ...
    (entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          ...
          if (
            (entry.target.getAttribute("gnta-high") === "on") !==
            highlightEnabled
          ) {
            if (highlightEnabled) {
          ...
          if (
            (entry.target.getAttribute("gnta-var") === "org") !==
              showOriginal ||
            (!showOriginal &&
              entry.target.getAttribute("gnta-var") !== currentVariant)
          ) {
            if (showOriginal) {
...
function convPage(nodeList) {
  ...
  convObserver = new IntersectionObserver(
    (entries) => {
      const start = performance.now();
      nodesHand = 0;
      charsHand = 0;
      ...
      console.log(
        `Handled   ` +
          nodesHand.toString().padStart(5, " ") +
          ` nodes with ` +
          charsHand.toString().padStart(7, " ") +
          ` chars in ` +
          duration.toFixed(2).toString().padStart(7, " ") +
          ` ms.`,
      );
    },
    {
...
function convNode(node) {
  ...
  if (node.nodeName === "GLOBALNOTETA_W_NODE" || converted !== original) {
    const c_node = document.createElement(
      `GlobalNoteTA_c_node_${currentVariant}`,
    );
...
function convText(text) {
  ...
  let i = 0;
  while (i < text.length) {
    // todo: rewrite and use strstr
    let matched = false;
    for (
      let len = Math.min(longestKey.length, text.length - i);
      len > 0;
      len--
    ) {
      const phrase = text.substr(i, len);
      if (table[phrase]) {
        const replacement = Array.isArray(table[phrase])
          ? table[phrase][0]
          : table[phrase]; // isArray should always be false, will confirm later
...
console.log(new Date().toISOString());
console.log(
  "GlobalNoteTA extension started running on page:",
